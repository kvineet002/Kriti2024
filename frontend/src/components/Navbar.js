  ...
  return (
    <div>
          ...
          <div className={`${isOpen ? `block` : `hidden`} w-full block flex-grow lg:flex lg:items-center lg:w-auto`}>
            <div className="text-sm lg:flex-grow">
              <form className="flex">
                ...
                <button type="submit" className="flex items-center bg-gray-100 bg-opacity-25 hover:bg-gray-200 hover:bg-opacity-50 text-white font-semibold py-2 px-4 rounded-lg">
            ...
            <div>
              <Link to='/profile/2' className="block mt-4 lg:inline-block lg:mt-0 text-white hover:underline mr-4">
                Profile
              </Link>
              <Link to='/project/2' className="block mt-4 lg:inline-block lg:mt-0 text-white hover:underline mr-4">
                Projects
              </Link>
              <Link to='/courses/2' className="block mt-4 lg:inline-block lg:mt-0 text-white hover:underline">
                Courses
              </Link>
            </div>
          </div>
      ...
      <div className="navbar fixed  top-0 lg:flex items-center justify-between flex-wrap w-full pl-[10px] bg-[#0c0c0c97] h-16  hidden ">
        <Link to="/" className="uppercase text-white font-semibold w-15 text-2xl flex justify-center items-center cursor-pointer">
          Collampus
        </Link>
        <nav className="flex flex-wrap justify-start w-[25%] items-center">
          <Link to="/profile/2" className="text-[#ffffff] font-[700] p-[10px] hover:underline cursor-pointer mx-1">People</Link>
          <Link to="/project/1" className="text-[#ffffff] font-[700] p-[10px] hover:underline cursor-pointer mx-1">Projects</Link>
          <Link className="text-[#ffffff] font-[700] p-[10px] hover:underline cursor-pointer mx-1">Courses</Link>
        </nav>
        <div className="w-[35%] pl-[50px]">
          <div className="flex justify-end items-center gap-2 pr-3 pl-0.5 h-8 rounded-[25px] bg-[#ffffffa5] ml-5 w-[85%] max-w-[400px]">
            <div className=" flex justify-center items-center bg-[#fffffff7] rounded-[50%]">
              <img src="/search.svg" height="15px" width="15px" className="h-[95%] w-[95%] mt-1 " />
            </div>
            <input
              type="text"
        ...
        {loggedIn ? (
          <Link to="/profile/2" className=" mx-4 bg-white text-black rounded-[25rem] text-center font-[700] cursor-pointer w-[5%] h-[30px] my-auto pt-[2px]">Profile</Link>
        ) : (
          <Link to="/login" className=" mx-4 bg-white text-black rounded-[25rem] text-center font-[700] cursor-pointer w-[5%] h-[30px] my-auto pt-[1px] ">
            Login
          </Link >
        )}
      </div>
